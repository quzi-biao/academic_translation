...
export async function summarizeAcademicDocument(mdContent) {
...
export async function buildTranslationPrompt(summary) {
  return chatText([
...
export async function translateBlockText(text, translationPrompt, documentSummary) {
  if (!text?.trim()) return '';
  return chatText([
    { role: 'system', content: translationPrompt },
    { role: 'user', content: `文献摘要上下文：\n${documentSummary}\n\n请翻译这个 block。只输出译文，不要解释。需要保留 Markdown 语义、公式、代码、引用编号和专有名词一致性。\n\n[Block]\n${text}` },
  ], { maxTokens: Math.min(4096, Math.max(800, Math.ceil(text.length * 2.2))) });
}
